...
func (c *Collection) updateIndexes(key string, doc Document) {
	...
	for field, index := range c.indexes {
		c.updateIndex(index, key, doc, field)
	}
}

func (c *Collection) updateIndex(index *Index, key string, doc Document, fieldName string) {
	valueToInsert, ok := doc.Fields[fieldName].Value.(string)
	if !ok || doc.Fields[fieldName].Type != DocumentFieldTypeString {
		return
	}
	...
	for _, entry := range index.Sorted {
		_, ok := entry.Document.Fields[fieldName].Value.(string)
		if !ok {
			continue // Пропускаємо, якщо тип не string (хоча цього не повинно бути)
		}
		if !removed && entry.Key == key {
			removed = true
			continue
		}
	...
	insertIndex := sort.Search(len(filtered), func(i int) bool {
		v, ok := filtered[i].Document.Fields[fieldName].Value.(string)
...
func (c *Collection) removeFromIndexes(key string, doc Document) {
	...
	for _, index := range c.indexes {
		c.removeFromIndex(index, key)
	}
}

func (c *Collection) removeFromIndex(index *Index, key string) {
	filtered := index.Sorted[:0]
